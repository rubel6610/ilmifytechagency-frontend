...
const DEPARTMENT_SKILLS: Record<string, string[]> = {
  MANAGEMENT: ['Leadership', 'Strategic Planning', 'Budgeting', 'Team Management', 'Decision Making', 'Project Management', 'Communication'],
  HUMAN_RESOURCE: ['Recruitment', 'Employee Relations', 'Training & Development', 'Performance Management', 'HR Policies', 'Compensation & Benefits', 'Conflict Resolution'],
  CMS: ['WordPress', 'Content Strategy', 'SEO', 'Web Analytics', 'UI/UX Design', 'HTML/CSS', 'JavaScript'],
  CUSTOM_DEVELOPMENT: ['JavaScript', 'TypeScript', 'React', 'Node.js', 'Python', 'Java', 'PHP', 'Database Design', 'API Development', 'System Architecture'],
  SHOPIFY: ['Shopify Development', 'Liquid', 'E-commerce', 'Theme Customization', 'App Integration', 'Payment Gateways', 'Store Optimization'],
  MARKETING: ['Digital Marketing', 'SEO/SEM', 'Content Creation', 'Social Media', 'Analytics', 'Email Marketing', 'Campaign Management', 'Brand Strategy'],
  SALES: ['Negotiation', 'Client Relations', 'CRM', 'Sales Strategy', 'Prospecting', 'Presentation Skills', 'Market Research'],
  SUPPORT: ['Customer Service', 'Technical Support', 'Troubleshooting', 'Communication', 'Problem Solving', 'Product Knowledge', 'Ticketing Systems'],
};
...
export default function AddMemberModal({ 
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'employeeId') {
      if (value && existingEmployeeIds.includes(value)) {
        setErrorMessage('⚠️ This Employee ID already exists. Please use a different ID.');
      } else if (value && !/^iLM-C-\d+$/.test(value)) {
        setErrorMessage('⚠️ Employee ID format should be iLM-C-[number] (e.g., iLM-C-63)');
      } else {
        setErrorMessage('');
      }
    } else {
  ...
  const validateForm = (): boolean => {
    ...
    if (!employeeIdRegex.test(formData.employeeId)) {
      setErrorMessage('❌ Employee ID format should be iLM-C-[number] (e.g., iLM-C-63)');
      return false;
    }

    if (existingEmployeeIds.includes(formData.employeeId)) {
      setErrorMessage('❌ This Employee ID already exists. Please use a different ID.');
      return false;
    }
    ...
    if (formData.linkedin && !formData.linkedin.startsWith('https://linkedin.com/') && 
      ...
      setErrorMessage('⚠️ LinkedIn URL should start with https://linkedin.com/ or https://www.linkedin.com/');
      return false;
    }

    if (formData.facebook && !formData.facebook.startsWith('https://facebook.com/') && 
        !formData.facebook.startsWith('https://www.facebook.com/')) {
      setErrorMessage('⚠️ Facebook URL should start with https://facebook.com/ or https://www.facebook.com/');
      return false;
    }

    if (formData.instagram && !formData.instagram.startsWith('https://instagram.com/') && 
        !formData.instagram.startsWith('https://www.instagram.com/')) {
      setErrorMessage('⚠️ Instagram URL should start with https://instagram.com/ or https://www.instagram.com/');
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? `❌ ${error.message}` : '❌ An error occurred while adding member');
    } finally {
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
              <div className="p-6 space-y-3">
                ...
                {errorMessage && (
                  ...
                  >
                    <span className="text-lg">{errorMessage.startsWith('❌') ? '❌' : '⚠️'}</span>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">JPG, PNG — max 5MB (Optional)</p>
                      <button 
                        type="button" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                      ...
                      {formData.employeeId && (
                        <div className="absolute right-3 top-1/2 -translate-y-1/2">
                          {existingEmployeeIds.includes(formData.employeeId) ? (
                            <span className="text-[10px] font-bold text-red-600 uppercase">✗ Taken</span>
                    ...
                    <p className="text-xs text-slate-500 mt-1">Format: iLM-C-[number]</p>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-900 mb-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-slate-900 mb-2">
                      Position <span className="text-slate-500 font-normal">(Optional)</span>
                    </label>
                    <input 
                      type="text" 
                      name="position" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-900 mb-2">
                    Blood Group <span className="text-slate-500 font-normal">(Optional)</span>
                  </label>
                  <div className="relative">
                    <button
                      type="button"
                      ...
                      className={`w-full px-4 py-2 rounded-lg font-semibold text-sm text-left border-2 flex items-center justify-between ${
                        isBloodGroupOpen ? 'bg-gradient-to-r from-[#0ddaa0] to-[#8ce064] text-white border-[#0ddaa0]' : 'bg-white border-slate-200 text-slate-900'
                      }`}
                    >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-900 mb-2">
                    LinkedIn <span className="text-slate-500 font-normal">(Optional)</span>
                  </label>
                  <input 
                    type="url" 
                    name="linkedin" 
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-900 mb-2">
                    Facebook <span className="text-slate-500 font-normal">(Optional)</span>
                  </label>
                  <input 
                    type="url" 
                    name="facebook" 
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-900 mb-2">
                    Instagram <span className="text-slate-500 font-normal">(Optional)</span>
                  </label>
                  <input 
                    type="url" 
                    name="instagram" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-900 mb-2">
                    Joining Date <span className="text-slate-500 font-normal">(Optional)</span>
                  </label>
                  <input 
                    type="date" 
                    name="startDate" 
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-900 mb-2">
                    Experience <span className="text-slate-500 font-normal">(Auto-calculated)</span>
                  </label>
                  <div className="w-full px-4 py-2 border-2 border-slate-200 rounded-lg bg-slate-50 text-sm text-slate-700">
                    {formData.experience} {formData.experience === 1 ? 'year' : 'years'} (calculated from joining date)
                  </div>
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-900 mb-2">
                  Skills <span className="text-slate-500 font-normal">(Optional, max 5)</span>
                </label>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-900 mb-2">
                    Member Description <span className="text-slate-500 font-normal">(Optional)</span>
                  </label>
                  <textarea 
