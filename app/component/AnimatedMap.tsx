...
// Type-safe GeoJSON data
const worldDataTyped = worldData as FeatureCollection<Geometry>;
...
const AnimatedMap = () => {
  ...
  // Use properly typed feature
const geojsonStyle = (feature?: Feature<Geometry, { name?: string }>) => {
  // Default style when feature is undefined
  const baseStyle = {
    fillColor: "#1e293b",
    weight: 1,
    opacity: 0.5,
  ...
  const isHighlighted = highlightedCountries.some(
    (c) =>
      (feature.id && c.id === feature.id) ||
      (feature.properties?.name && c.name === feature.properties.name)
  );

  return {
    ...baseStyle,
    fillColor: isHighlighted ? "#22c55e" : baseStyle.fillColor,
    fillOpacity: isHighlighted ? 0.6 : baseStyle.fillOpacity,
  };
};

  return (
    <section className="w-full bg-[#020617] pb-8 pt-8 sm:pt-12 md:pt-16 lg:pt-20">
      <div className="max-w-400 mx-auto text-center mb-6 sm:mb-8 md:mb-12 px-4">
        ...
        <p className="text-[16px] text-gray-400">
          Delivering digital solutions to clients in the world’s most impactful economies.
        </p>
      </div>
      ...
      <div className="relative h-87.5 sm:h-100 md:h-125 lg:h-150 xl:h-187.5 max-w-400 px-2 sm:px-4 md:px-8 mx-auto">
        <div className="h-full w-full rounded-xl sm:rounded-2xl md:rounded-[2.5rem] lg:rounded-[3rem] overflow-hidden shadow-2xl">
          ...
          >
            ...
            {/* ✅ Use typed worldData here */}
            <GeoJSON data={worldDataTyped} style={geojsonStyle} />
