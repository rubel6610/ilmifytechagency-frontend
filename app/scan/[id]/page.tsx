...
export default function ScanResultPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetTeamMembersQuery({ limit: 1000 });

  const [member, setMember] = useState<any>(null);

  useEffect(() => {
    if (teamResponse?.data) {
      const foundMember = teamResponse.data.find(
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-4 text-center">
        ...
        <p className="text-slate-600">
          The employee ID "{decodeURIComponent(employeeId)}" does not exist or
          could not be loaded.
        </p>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8 flex items-center justify-center mt-10">
      <motion.div
      ...
      >
        ...
        <div className="pt-20 pb-8 px-6 text-center">
          <h1 className="text-2xl font-bold text-slate-900 mb-1 flex justify-center items-center gap-4">
            <span>{member.fullName} </span>
            {member.status === "ACTIVE" ? <BadgeCheck color="green"/> : <CircleOff/>}
          </h1>
          ...
          <div className="space-y-4 text-left">
            ...
            {member.email && (
              <a
                ...
                className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl hover:bg-[#0ddaa0]/5 transition-colors group"
              >
